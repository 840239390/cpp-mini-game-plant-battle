...
public:
	...
	{	
		...
		pos_img_winner_text.x = pos_img_winner_bar.x + (img_winner_bar.getwidth() - img_1P_winner.getwidth()) / 2;
		pos_img_winner_text.y = pos_img_winner_bar.y + (img_winner_bar.getheight() - img_1P_winner.getheight()) / 2;
	...
	{	
		...
		if (is_debug)
		{
			...
			outtextxy(15, 15, _T("已开启debug模式，按Q关闭"));
		}
...
private:
	...
	POINT pos_img_winner_bar = { 0 };		//结束动效背景位置
	POINT pos_img_winner_text = { 0 };		//结算动效文本位置
	int pos_x_img_winner_bar_dst = 0;		//结算动效背景的移动目标位置
	int pos_x_img_winner_text_dst = 0;		//结算动效文本的移动目标位置
